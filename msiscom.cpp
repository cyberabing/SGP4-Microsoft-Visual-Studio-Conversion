...
//------------------------------ begin ---------------------------------
        for (i = 1; i <= 25; i++)
          {
//            sav[i] = sv[i];
            csw.sw[i] = (int) fmod(sv[i],2);//DEBUG: added explicit casts to make conversion unambiguous.
            if ( (fabs((double)sv[i]) == 1)  ||  (fabs((double)sv[i]) == 2) )//DEBUG: added explicit casts to make function call unambiguous.
																			//DEBUG: added parentheses to make operator precedence unambiguous.
                csw.swc[i] = 1;
              else
                csw.swc[i] = 0;
          }
...
//------------------------------ begin ---------------------------------
        ...
        if ( (dm <= 0.0)  ||  (dd <= 0.0 ) )//DEBUG: added parentheses to make operator precedence unambiguous.
          {
...
//------------------------------ begin ---------------------------------
        e1 = (alt-zh)/h1;
        e2 = (alt-zh)/h2;
        if ((e1 > 70.0) || (e2 > 70.0))//DEBUG: added parentheses to make operator precedence unambiguous.
            ccor1 = 0.0;
          else
          {
            if ((e1 < -70.0) && (e2 < -70.0)) //DEBUG: added parentheses to make operator precedence unambiguous.
          ...
          if (xm != 0.0)
            {
              ...
              if ((expl > 50)  ||  (tt <= 0.0))//DEBUG: added parentheses to make operator precedence unambiguous.
                expl = 50.0;
              ...
              if (alt < za)
                {
                  ...
                  expl = gamm*yi;
                  if ((expl > 50.0) || (tz <= 0.0))//DEBUG: added parentheses to make operator precedence unambiguous.
                      expl = 50.0;
...
//------------------------------ begin ---------------------------------
        ...
        while (khi-klo > 1)
          {
            k = (int)( (khi+klo) * 0.5 );//DEBUG: added explicit casts to make conversion unambiguous.
            if (xa[k] > x)
                khi = k;
              else
                klo = k;
