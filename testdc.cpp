...
//#include "sgp4ext.h" // Alek Lidkte, 23/07/2014: if this is included some compiler errors will show up due to variable names being defined as compiler macros in this file.
#include "sgp4io.h"
#include "sgp4unit.h"
...
#define NO_CARTESIAN_OBSERVATIONS 75 // Size of the obscarr array.
...
FILE *outfile, *outfile1, *outfile2;
   ...
   int lastob;
   int statesize=7; //HACK: fix statesize to 7.
   char batchmode;
...
void func()
{ //TODO: fix commandline argument parsing for MSVC c.f. Borland compilers. I have the code for this just need to sit down and do it, Alek Lidtke, 23/07/2014.
   ...
   //HACK: temporarily use hard-coded arguments to test code compilation and execution.
   typeans    = 's'; // Back substitution, can also do SVD with s.
   typeodrun  = 't'; // Process TLEs.
   statetype  = 'v'; // Vectors, can also be t for TLEs or r for az-el.
   percentchg = 0.001;    // 0.001
   deltaamtchg= 0.0000001;    // 0.0000001
   rmsepsilon = 0.0002;    // 0.001
   strcpy( fname, "spaceTrackTLEs_23_10_2013.txt");
   lastob     = NO_CARTESIAN_OBSERVATIONS;    // 200
}
...
void processrazel()
  {
     obsrec obsrecarr[ NO_CARTESIAN_OBSERVATIONS ];
     elsetrec satrec;
     elsetrec satrecx;
     long search_satno = 0;
     ...
     std::vector< std::vector<double> > trans(3,std::vector<double>(3,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
     ...
     std::vector< std::vector<double> > x(statesize,std::vector<double>(1,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
     std::vector< std::vector<double> > scalef(statesize,std::vector<double>(1,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
     std::vector< std::vector<double> > dx(statesize,std::vector<double>(1,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
     std::vector< std::vector<double> > atwai(statesize,std::vector<double>(1,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
     std::vector< std::vector<double> > atwa(statesize,std::vector<double>(1,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
     std::vector< std::vector<double> > atwb(statesize,std::vector<double>(1,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
...
void processxyz()
  {
     ...
     std::vector< std::vector<double> > scalef(statesize,std::vector<double>(1,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
     std::vector< std::vector<double> > dx(statesize,std::vector<double>(1,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
     std::vector< std::vector<double> > atwai(statesize,std::vector<double>(statesize,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
     std::vector< std::vector<double> > atwa(statesize,std::vector<double>(statesize,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
     std::vector< std::vector<double> > atwb(statesize,std::vector<double>(1,0.0)); //DEBUG: updated the vector<vector<double>> constructor.
...
void processtle()
   {
     ...
     char str[2];
     char typerun;
	 char typeinput = 'm'; //DEBUG: set typeinput
     gravconsttype whichconst;
          ...
          while (feof(infile) == 0)
            {
              ...
              if (feof(infile) == 0)
                {
                  fgets( longstr2,130,infile); //TODO: 1) read a TLE from input file and temporarily save as satrec.
               ...
               // ----------------- and setup obsrecarr ------------------------
               for(ii=0; ii <= lastob; ii++) //TODO: 2) create state vectors spawned from the TLE around it separated by dtmin (scaled with orbital period). Save those in obsrecarr.
                 {
                   currobsrec.dtmin = dtmin;
               ...
               // setup nominal vector -----------------------------------------
			   //TODO: 3) nomianal TLE, i.e. the one read from the file, is now stored in satrecx.
               satrecx = satrec;    // store the answer for later
               /*
               ...
               */
               ...
               sgp4 (whichconst, satrecx,  0.0, rnom,  vnom); //TODO: 4) get nominal TLE position as rnom and velocity as vnom.
               for (i= 0; i < 6; i++) //TODO: 5) start finding the reproduced TLE from the iitial state of the original TLE.
                 {
                   if (i < 3)
                       xnom[i] = rnom[i];
                     else
               ...
               if (statesize > 6) //TODO: 6) solve for B* as well.
                   xnom[6] = 0.0001; // set initial guess as it changes in state2satrec
                 else
                   satrec.bstar = 0.0001;  
               state2satrec( xnom, scalef, 'v', statesize, eTo, satrec ); // force this method to assign values

               satrec.jdsatepoch = satrecx.jdsatepoch;  // be sure to set the epoch of the temporary satellite record!
               sgp4init( whichconst, opsmode, satrec.satnum, satrec.jdsatepoch-2433281.5, satrec.bstar,
                         satrec.ecco, satrec.argpo, satrec.inclo, satrec.mo, satrec.no,
                         satrec.nodeo, satrec);
               // call the propagator to get the initial state vector value
               sgp4 (whichconst, satrec,  0.0, rnom,  vnom); //TODO: 7) call the propagator to get the initial state vector value for the reproduced TLE stored in satrec
